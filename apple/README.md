...
### 2026-07-06 — Sonnet 5 fix round vs PR #30 review findings (`cursor/native-quick-wins`, Claude Code orchestrator + 5 parallel Sonnet sub-agents)
Five disjoint-file bundles fixing confirmed review findings on quick-wins A–E. **(1) PageClassifier:** Person must be top-level JSON-LD or `mainEntity`/`about` target (Yoast `@graph` author no longer flips pages to `individual`); foundry keywords whole-word/domain-label only; social links exact-host/dot-suffix + share/intent-path skip + `rel=me` two-pass priority, blind mastodon fallback deleted; parser dedup (`OGResult.ogType`, shared static regexes, JSON-LD parsed once). **(2) Review strip:** `needsReview` cleared when the last suggestion is consumed (accept + dismiss) AND `isEligible` requires non-empty suggestions past `text_done`; `ensureAwaitingReviewFixtures` no-ops unless the container is in-memory (screenshot fixtures were mutating + CloudKit-syncing real rows); per-row JSON decode cached. **(3) Search:** real persistent blob cache keyed `(id, updatedAt)` in `ItemSearchBlob`; `bodyMarkdown` searchable; single `ItemFilter.apply` pass; strip filter cached with refresh-on-dismiss + cheap `needsReview` bool-array onChange (gate round 2 — first pass left the cache stale on dismiss). **(4) Revisit:** `DetailView` `.onAppear` → `.task(id: item.persistentModelID)` so the persistent panel DetailView records each selection change. **(5) Why-saved chips:** accept hit area restored to full padded capsule via label-level padding + `.contentShape`, ✕ separately padded with divider, `role: .destructive`. **138 → 154 tests** green macOS; iOS Simulator build green. Follow-ups flagged: dismiss-chip confirmation/undo; chip touch targets ~33–36pt (under 44pt ideal); scrolled-to-chips iOS screenshot unverified (macOS verified, no platform-conditional code).
