...
**Timeline bars drifted up on large libraries only (2026-07-06):** `.offset(y:)`-positioned bars inside a plain `ZStack` don't affect layout, so the ZStack's intrinsic height = tallest bar. When one week's bar exceeds the viewport height (real ~163-item store; never the 20-item clean store), the enclosing `.frame(maxHeight:.infinity)` vertically centers the oversized stack → every bar drifts up by (contentHeight − viewport)/2. Fix: wrap the bars layer in `GeometryReader` (fills the space, topLeading origin, children contribute no intrinsic size). **Testing lesson: clean-store screenshot fixtures mask store-size-dependent layout bugs — verify layout work against a production-scale store (or a many-week fixture) too.** Diagnosed via a temporary `-diagTimeline` launch-arg print (removed).
