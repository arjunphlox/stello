...
| Entry | Summary |
|---|---|
| [project_native_apple_app.md](project_native_apple_app.md) | Native SwiftUI app under apple/Stello — local-first SwiftData+CloudKit, on-device AFM enrichment, XcodeGen harness, macOS inset-glass header (Sketch pattern), seed dedupe + broken-shell ItemImage gotchas; quick-wins A–E shipped Jul 2026 + PR #30 fix round (cached-@State staleness trap, `.task(id:)` over `.onAppear` in persistent panels, in-memory-only screenshot fixtures, structural classifier signals); timeline/header frozen until explicit OK (failed nudge session reverted) |
