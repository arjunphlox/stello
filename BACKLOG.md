...
| **Native quick-wins sprint** — revisit tracking (`lastOpenedAt`/`openCount`), why-saved→intent facet (accept chips → intent tags + filter section), awaiting-review strip, search over snippets/why-saved/metadata, kind classification + deterministic social/metadata extraction at capture. Paste-ready prompt: `docs/plans/native-quick-wins-sprint.md` | Cursor | iOS/macOS | Composer | Open |
