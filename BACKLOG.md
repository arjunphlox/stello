...
| **Run `recover-errored-items.js` after PR #17 deploys** — 379 items stuck in `enrichment_status='error'` with intact R2 images; pilot `--apply --limit 3`, confirm they heal (watch `enrichment_error`), then `--limit 1000`. Gated on the PR being deployed so re-enrichment benefits from the fixes | Claude Code | Desktop | Sonnet | Open |
