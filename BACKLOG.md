...
| **Re-enable vision enrichment after macOS beta update** — when a macOS build ships the FoundationModels image `Attachment` API, flip `FoundationModelsEnricher.imageAttachmentSupported` to true and verify with a manual Enrich on a cover-bearing item (crash context: docs/memory/project_native_apple_app.md build-harness section) | Cursor | macOS | Composer | Open |
